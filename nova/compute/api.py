...
class API(base.Base):
    ...
    def hot_extend_vdisk(self, context, instance, disk, size, granularity):
        ...
        need_gb = int(size) - instance.root_gb
        if need_gb > free_gb:
            return 'There is no enough free disk for hot extend!'

        instance.task_state = task_states.HOT_EXTENDING
        instance.save()
        self.compute_task_api.hot_extend_vdisk(context, instance, disk, size, granularity)
        return None
    ...
    @check_instance_cell
    @check_instance_state(vm_state=[vm_states.ACTIVE, vm_states.ERROR])
    def set_actual_total_memory(self, context, instance, size, granularity="MB", do_cast=True):
        """Set actual total memory size for the specified instance."""
        self.compute_rpcapi.set_actual_total_memory(context, instance,
                                                    size, granularity,
                                                    do_cast=do_cast)
    ...
    def get_actual_total_memory(self, context, instance):
        """Get actual total memory size of the specified instance."""
        return self.compute_rpcapi.get_actual_total_memory(context,
                                                           instance)
    ...
    def get_max_memory(self, context, instance):
        """Get actual total memory size of the specified instance."""
        return self.compute_rpcapi.get_max_memory(context, instance)
    ...
    def get(self, context, instance_id, want_objects=False,
        ...
        compute_svc_status=''
        if instance.host:
            svc = objects.Service.get_by_compute_host(context,instance.host)
            compute_svc_status = (self.servicegroup_api.service_is_up(svc) and "up" ) or "down"
        setattr(instance,'host-compute-service-status',compute_svc_status)
    ...
    def resize(self, context, instance, flavor_id=None, clean_shutdown=True,
        ...
        new_instance_type.extra_specs = current_instance_type.extra_specs
...
class HostAPI(base.Base):
    ...
    def host_ksm_ctl(self, context, host_name, action):
        result = self.rpcapi.host_ksm_ctl(context, action=action,
                host=host_name)
        return result

    def enable_zram(self, context, host_name, action):
        result = self.rpcapi.enable_zram(context, host=host_name,
                action=action)
        return result

    def disable_zram(self, context, host_name, action):
        result = self.rpcapi.disable_zram(context, host=host_name,
                action=action)
        return result

    def get_zram_status(self, context, host_name, action):
        result = self.rpcapi.get_zram_status(context, host=host_name,
                action=action)
        return result
...
class AggregateAPI(base.Base):
    ...
    def is_safe_to_update_az(self, context, metadata, aggregate,
        ...
        if 'availability_zone' in metadata:
            ...
            if host_aggregates:
               msg = _("One or more hosts already in host_aggregate(s) "
                        "%s") % host_aggregates
               self._raise_invalid_aggregate_exc(action_name, aggregate.id,
                                                  msg)
            conflicting_azs = [
