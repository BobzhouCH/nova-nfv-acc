...
class ComputeAPI(object):
    ...
    def host_ksm_ctl(self, ctxt, action, host):
        version = '4.0'
        cctxt = self.client.prepare(server=host, version=version)
        return cctxt.call(ctxt, 'host_ksm_ctl', action=action)

    def enable_zram(self, ctxt, host, action):
        version = '4.0'
        cctxt = self.client.prepare(server=host, version=version)
        return cctxt.call(ctxt, 'enable_zram', action=action)

    def disable_zram(self, ctxt, host, action):
        version = '4.0'
        cctxt = self.client.prepare(server=host, version=version)
        return cctxt.call(ctxt, 'disable_zram', action=action)

    def get_zram_status(self, ctxt, host, action):
        version = '4.0'
        cctxt = self.client.prepare(server=host, version=version)
        return cctxt.call(ctxt, 'get_zram_status', action=action)
    ...
    def hot_extend_vdisk(self, ctxt, instance, disk, size, granularity):
        msg_args = {'instance': instance,
                    'disk': disk,
                    'size': size,
        ...
        cctxt = self.client.prepare(server=_compute_host(None, instance),
                version=version)
        result = cctxt.call(ctxt, 'hot_extend_vdisk', **msg_args)
        return jsonutils.loads(result)

    def set_actual_total_memory(self, ctxt, instance, size, granularity="MB",
                                do_cast=True):
        msg_args = {'instance': instance,
                    'size': size,
                    'granularity': granularity}
        ...
        cctxt = self.client.prepare(server=_compute_host(None, instance),
                version=version)
        rpc_method = cctxt.cast if do_cast else cctxt.call
        return rpc_method(ctxt, 'set_actual_total_memory', **msg_args)

    def get_actual_total_memory(self, ctxt, instance):
        msg_args = {'instance': instance}
        version = '4.0'
        cctxt = self.client.prepare(server=_compute_host(None, instance),
                version=version)
        result = cctxt.call(ctxt, 'get_actual_total_memory', **msg_args)
        return result

    def get_max_memory(self, ctxt, instance):
        msg_args = {'instance': instance}
        version = '4.0'
        cctxt = self.client.prepare(server=_compute_host(None, instance),
                                    version=version)
        return cctxt.call(ctxt, 'get_max_memory', **msg_args)
