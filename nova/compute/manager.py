...
CONF.import_opt('bm_mgmt_uuid', 'nova.api.openstack.compute.servers')
...
def host_service_status(service_name):
    return os.system('systemctl status %s' % service_name)


def host_service_start(service_name):
    return os.system('sudo nova-rootwrap /etc/nova/rootwrap.conf systemctl start %s' % service_name)
...
class ComputeManager(manager.Manager):
    ...
    def __init__(self, compute_driver=None, *args, **kwargs):
        ...
        self.use_legacy_block_device_info = \
            ...
            #self.pssr = pssr(self.conductor_api)

    def reset(self):
        LOG.info(_LI('Reloading compute RPC API'))
        compute_rpcapi.LAST_VERSION = None
        self.compute_rpcapi = compute_rpcapi.ComputeAPI()
    def nfvi_pssr_conf(self, context, req):
        LOG.info("nfvi_pssr_conf,req = %s" % req)
        return self.pssr.pssr_conf(req)
    def get_port_info(self, context, req):
        LOG.info("get_port_info,req = %s" % req)
        LOG.info("ethinfo=%s" % self.pssr.ethinfo)
        return self.pssr.ethinfo
    def get_intf_info(self, context, req):
        LOG.info("get_intf_info,req = %s" % req)
        return self.pssr.intfinfo    
    def get_nova_compute_pid(self,context,req):
        LOG.info("get_nova_compute_pid, pid = %s" % self.pssr.pid)
        return self.pssr.pid
    ...
    def hot_extend_vdisk(self, context, instance, disk, size, granularity):
        disk = disk.split('/')[2]
        result = {'result': 'ok', 'msg': '', 'disk': disk}
        if granularity != "GB":
            result['result'] = 'fail'
            result['msg'] = 'granularity should be "GB"!'
            LOG.error("hot_extend_vdisk: %s", result)
            return jsonutils.dumps(result)

        try:
            info = self.driver.block_info(instance, disk)
        except Exception as e:
            result['result'] = 'fail'
            result['msg'] = '%s' % e
            LOG.error("hot_extend_vdisk(%s): %s", type(e), result)
            return jsonutils.dumps(result)
    
        size_in_kilobytes = int(size)*1024*1024 
        if size_in_kilobytes < info['Capacity(KB)']:
            result['result'] = 'fail'
            result['msg'] = 'Make sure the new size is larger than old size!'
            LOG.error("hot_extend_vdisk: %s", result)
            return jsonutils.dumps(result)

        try:
            self.driver.block_resize(instance, disk, size_in_kilobytes)
        except Exception as e:
            result['result'] = 'error'
            result['msg'] = '%s' % e
            LOG.error("hot_extend_vdisk(%s): %s", type(e), result)
    ...
    @reverts_task_state
    @wrap_instance_fault
    def set_actual_total_memory(self, context, instance, size, granularity):
        """Set actual total memory for an active instance"""
        self.driver.set_memory(instance, size, granularity)

    @wrap_exception()
    @reverts_task_state
    @wrap_instance_fault
    def get_actual_total_memory(self, context, instance):
        """Get actual total memory of an active instance"""
        """
           One example of the return result is as below,
           the granularity is KB:
           {
            'actual': <a number>,
            'available': <a number>,
            'major_fault': <a number>,
            'minor_fault': <a number>,
            'rss': <a number>,
            'swap_in': <a number>,
            'swap_out': <a number>,
            'unused': <a number>
        ...
        """
        return self.driver.get_memory(instance)

    @wrap_exception()
    @reverts_task_state
    @wrap_instance_fault
    def get_max_memory(self, context, instance):
        """Get max memory of the specified instance"""
        return self.driver.get_max_memory(instance)
    ...
    def host_ksm_ctl(self, context, action):
        class HostKSMCtl(object):
            file_run = "/sys/kernel/mm/ksm/run"
            ksm_status = {-1: "error", 0: "Off", 1: "On", 2: "Off"}

            @classmethod
            def show(cls):
                result = -1
                with open(cls.file_run, "r") as f:
                    result = f.read()
                return {'status': '%s' % cls.ksm_status[int(result)]}

            @classmethod
            def handle_onoff(cls, n):
                os.system('echo %d > ~/run' % n)
                os.system('sudo nova-rootwrap /etc/nova/rootwrap.conf cp ~/run %s' % cls.file_run)
                os.system('rm ~/run')
                with open(cls.file_run, "r") as f:
                    result = f.read()
                return int(result) == n
            ...
            @classmethod
            def enable(cls):
                if host_service_status('ksm') != 0:
                    r = host_service_start('ksm')
                    if r != 0:
                        return {'result': 'error: failed to start ksm service'}
                return {'result': 'ok' if cls.handle_onoff(1) else 'error: failed to enable ksm'}

            @classmethod
            def disable(cls):
                return {'result': 'ok' if cls.handle_onoff(0) else 'error: failed to disable ksm'}

        method = getattr(HostKSMCtl, action)
        result = method()
        return result

    def reset_zram(self, num):
        f1 = os.open("/tmp/zram%s.reset" % num, os.O_RDWR | os.O_CREAT)
        os.chmod("/tmp/zram%s.reset" % num, 0o222)
        os.write(f1, "1")
        os.close(f1)
        os.system("sudo nova-rootwrap /etc/nova/rootwrap.conf \
            cp -f /tmp/zram%s.reset /sys/block/zram%s/reset" % (num, num))
        os.remove("/tmp/zram%s.reset" % num)
        return {'result': 'OK'}

    def enable_zram(self, context, action):
        ratio_total = 0.25
        ratio_free = 0.40

        cpu_nums = min(multiprocessing.cpu_count(), 16)
        mem_total = psutil.virtual_memory().total
        mem_free = psutil.virtual_memory().free
        mem_zram = min(mem_total * ratio_total, mem_free * ratio_free)
        disksize = int(mem_zram / cpu_nums)

        os.system("sudo nova-rootwrap /etc/nova/rootwrap.conf "\
            "modprobe zram num_devices=%d" % cpu_nums)

        for i in range(cpu_nums):
            f2 = os.open("/tmp/zram%s.disksize" % i, \
                         os.O_RDWR | os.O_CREAT)
            os.chmod("/tmp/zram%s.disksize" % i, 0o666)
            if disksize != 0:
                os.write(f2, "%s" % str(disksize))
            os.close(f2)
            self.reset_zram(i)
            os.system("sudo nova-rootwrap /etc/nova/rootwrap.conf \
                cp -f /tmp/zram%s.disksize /sys/block/zram%s/disksize" % (i, i))
            os.remove("/tmp/zram%s.disksize" % i)
            os.system("sudo nova-rootwrap /etc/nova/rootwrap.conf \
                mkswap /dev/zram%s" % i)  # zram{0} format(i)
            os.system("sudo nova-rootwrap /etc/nova/rootwrap.conf \
                swapon -p 100 /dev/zram%s" % i)
        return {'result': 'OK'}

    def disable_zram(self, context, action):
        cpu_nums = multiprocessing.cpu_count()
        for i in range(cpu_nums):
            os.system("sudo nova-rootwrap /etc/nova/rootwrap.conf \
                swapoff /dev/zram%s" % i)
            self.reset_zram(i)
        return {'result': 'OK'}

    def get_zram_status(self, context, action):
        percent = psutil.virtual_memory().percent
        zram_status = {'status': 'Off',
                       'current_memory_usage_percent': percent}
        (status, output) = commands.getstatusoutput("swapon -s")
        words = re.split(r'[/\s\t\n]\t*', output)
        for word in words:
            pattern = re.compile(r'zram\d')
            if pattern.match(word):
                zram_status['status'] = 'On'
                break
        return zram_status
