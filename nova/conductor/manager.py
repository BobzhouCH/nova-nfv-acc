...
class ConductorManager(manager.Manager):
    ...
    def license_check(self, context, sn):
        return True
    ...
    def get_compute_node_port_list(self, context, host_id):
        port_list = db_api.nfv_port_get_by_node_id(context, host_id)
        return port_list
    def get_compute_node_interface_list(self, context, host_id):
        interface_list = db_api.nfv_interface_get_by_node_id(context, host_id)
        return interface_list

    def compute_node_get_all_by_host(self,context, host):
        return db_api.compute_node_get_all_by_host(context, host)

    def nfv_port_get_by_node_id(self,context, node_id):
        return db_api.nfv_port_get_by_node_id(context, node_id)

    def nfv_port_resource_update(self,context, node_id, nic_id, values):
        return db_api.nfv_port_resource_update(context, node_id, nic_id, values)

    def nfv_port_delete(self,context, node_id, nic_id):
        return db_api.nfv_port_delete(context, node_id, nic_id)

    def nfv_port_get_by_nic_id(self,context, node_id, nic_id):
        return db_api.nfv_port_get_by_nic_id(context, node_id, nic_id)

    def nfv_interface_delete(self,context, node_id, nic_id):
        return db_api.nfv_interface_delete(context, node_id, nic_id)

    def nfv_interface_get_by_nic_id(self,context, node_id, nic_id):
        return db_api.nfv_interface_get_by_nic_id(context, node_id, nic_id)

    def nfv_interface_resource_update(self,context, node_id, nic_id, values):
        return db_api.nfv_interface_resource_update(context, node_id, nic_id, values)

    def nfv_port_resource_create(self,context, values):
        return db_api.nfv_port_resource_create(context, values)

    def nfv_interface_get_by_node_id(self,context, node_id):
        return db_api.nfv_interface_get_by_node_id(context, node_id)

    def nfv_interface_resource_create(self,context, values):
        return db_api.nfv_interface_resource_create(context, values)
...
class ComputeTaskManager(base.Base):
    ...
    def hot_extend_vdisk(self, context, instance, disk, size, granularity):
        if granularity != "GB":
            ...
            instance.save()
            return
        if disk is not None and disk != instance.root_device_name:
            instance.task_state = None
            instance.save()
            return
        ...
        free_gb = compute_node.free_disk_gb
        need_gb = int(size_in_gb) - instance.root_gb
        if need_gb > free_gb:
            LOG.error("hot extend vdisk need_gb > free_gb: %s > %s", need_gb, free_gb)
            instance.task_state = None
            instance.save()
            return

        disk = instance.root_device_name
        result = self.compute_rpcapi.hot_extend_vdisk(context, instance, disk, size, granularity)
        if result['result'] == 'fail':
            LOG.error("hot extend vdisk compute_rpcapi result: %s", result)
            instance.task_state = None
            instance.save()
            return
        ...
        all_flavors = objects.Flavor.get_exactly(context, filters={'vcpus': old_flavor.vcpus, 'memory_mb': old_flavor.memory_mb, 'root_gb': size_in_gb})
        if all_flavors is None:
            flavorid = six.text_type(uuid.uuid4())
            values = {'root_gb': size_in_gb
                    , 'name': "%s-%s-%s" % (old_flavor.vcpus, old_flavor.memory_mb, size_in_gb)
                    , 'ephemeral_gb': old_flavor.ephemeral_gb
                    , 'memory_mb': old_flavor.memory_mb
                    , 'vcpus': old_flavor.vcpus
                    , 'swap': old_flavor.swap
                    , 'rxtx_factor': old_flavor.rxtx_factor
                    , 'is_public': old_flavor.is_public
                    , 'flavorid': flavorid
                    }
            new_flavor = objects.Flavor(context, **values)
            new_flavor.create()
            LOG.info("hot_extend_vdisk handle flavor: new flavor: %s", new_flavor)
        else:
            LOG.info("hot_extend_vdisk handle flavor: such flavor exist!")
            new_flavor = all_flavors

        instance.root_gb = size_in_gb
        instance.instance_type_id = new_flavor.id
        instance.flavor = new_flavor
        instance.task_state = None
        instance.save()

        compute_node.local_gb_used += need_gb
        compute_node.free_disk_gb -= need_gb
        compute_node.save()
        LOG.info("hot_extend_vdisk success! instance: %s", instance)
