...
def compute_node_statistics(context):
    ...
    result = model_query(context,
                         models.ComputeNode, (
                             ...
                             storage_func(models.ComputeNode.free_disk_gb),
                             func.sum(models.ComputeNode.current_workload),
                             func.sum(models.ComputeNode.running_vms),
                             storage_func(models.ComputeNode.disk_available_least),
                         ), read_deleted="no").\
...
@require_context
@main_context_manager.reader
def flavor_get_exactly(context, inactive=False, filters=None):
    filters = filters or {}
    read_deleted = "yes" if inactive else "no"
    query = _flavor_get_query(context, read_deleted=read_deleted)
    if 'vcpus' in filters:
        query = query.filter(
                models.InstanceTypes.vcpus == filters['vcpus'])
    if 'memory_mb' in filters:
        query = query.filter(
                models.InstanceTypes.memory_mb == filters['memory_mb'])
    if 'root_gb' in filters:
        query = query.filter(
                models.InstanceTypes.root_gb == filters['root_gb'])
...
@require_context
@main_context_manager.writer
def nfv_port_resource_create(context, values):
    nfv_port = models.NfvPortResource()
    nfv_port.update(values)
    nfv_port.save(context.session)
    return nfv_port


@main_context_manager.reader
def nfv_port_get_all(context):
    result = model_query(context, models.NfvPortResource).all()
    return result


@main_context_manager.reader
def nfv_port_get_by_node_id(context, compute_node_id):
    result = model_query(context, models.NfvPortResource).\
            filter_by(node_id=compute_node_id).\
            all()
    return result


def _nfv_port_node_nic_id_get(context, compute_node_id, nic_id):
    result = model_query(context, models.NfvPortResource).\
            filter_by(node_id=compute_node_id).\
            filter_by(nic_id=nic_id).\
            first()
    # if not result:
    #     raise exception.NfvPortNotFound(node=compute_node_id)
...
@oslo_db_api.wrap_db_retry(max_retries=5, retry_on_deadlock=True)
@pick_context_manager_writer
def nfv_port_resource_update(context, compute_node_id, nic_id, values):
    nfv_port = _nfv_port_node_nic_id_get(context, compute_node_id,
                                         nic_id)
    values['updated_at'] = timeutils.utcnow()
    datetime_keys = ('created_at', 'deleted_at', 'updated_at')
    convert_objects_related_datetimes(values, *datetime_keys)
    LOG.info("nfv_port_resource_update,compute_node_id= %s,nic_id = %s,nfv_port = %s" % (compute_node_id,nic_id,nfv_port))	
...
@pick_context_manager_writer
def nfv_port_delete(context, compute_node_id, nic_id):
    ret = model_query(context, models.NfvPortResource).\
                      filter_by(node_id=compute_node_id).\
                      filter_by(nic_id=nic_id).\
                      soft_delete(synchronize_session=False)
    if not ret:
        raise exception.NfvPortNotFound(host=compute_node_id)


@pick_context_manager_reader
def nfv_port_get_by_nic_id(context, node_id, nic_id):
    nfv_port_ref = _nfv_port_node_nic_id_get(context, node_id, nic_id)
...
@require_context
@main_context_manager.writer
def nfv_interface_resource_create(context, values):
    nfv_interface = models.NfvInterfaceResource()
    nfv_interface.update(values)
    nfv_interface.save(context.session)
    return nfv_interface


@main_context_manager.reader
def nfv_interface_get_all(context):
    result = model_query(context, models.NfvInterfaceResource).all()
    return result


@main_context_manager.reader
def nfv_interface_get_by_node_id(context, compute_node_id):
    result = model_query(context, models.NfvInterfaceResource).\
            filter_by(node_id=compute_node_id).\
            all()
    return result


def _nfv_interface_node_nic_id_get(context, compute_node_id, nic_id):
    result = model_query(context, models.NfvInterfaceResource).\
            filter_by(node_id=compute_node_id).\
            filter_by(nic_id=nic_id).\
            first()
    # if not result:
    #     raise exception.NfvInterfaceNotFound(node=compute_node_id)
...
@oslo_db_api.wrap_db_retry(max_retries=5, retry_on_deadlock=True)
@pick_context_manager_writer
def nfv_interface_resource_update(context, compute_node_id, nic_id, values):
    nfv_interface = _nfv_interface_node_nic_id_get(context, compute_node_id,
                                         nic_id)
    values['updated_at'] = timeutils.utcnow()
    datetime_keys = ('created_at', 'deleted_at', 'updated_at')
    convert_objects_related_datetimes(values, *datetime_keys)
...
@pick_context_manager_writer
def nfv_interface_delete(context, compute_node_id, nic_id):
    ret = model_query(context, models.NfvInterfaceResource).\
                      filter_by(node_id=compute_node_id).\
                      filter_by(nic_id=nic_id).\
                      soft_delete(synchronize_session=False)
    if not ret:
        raise exception.NfvInterfaceNotFound(host=compute_node_id)


@pick_context_manager_reader
def nfv_interface_get_by_nic_id(context, node_id, nic_id):
    nfv_interface_ref = _nfv_interface_node_nic_id_get(context, node_id, nic_id)

    return nfv_interface_ref
