...
class ComputeHostAPITestCase(test.TestCase):
    ...
    def test_enable_zram(self):
        self._mock_rpc_call('enable_zram',
                            host='fake_host',
                            action='fake_action')
        self.mox.ReplayAll()
        result = self.host_api.enable_zram(self.ctxt, 'fake_host', 'fake_action')
        self.assertEqual('fake-result', result)

    def test_disable_zram(self):
        self._mock_rpc_call('disable_zram',
                            host='fake_host',
                            action='fake_action')
        self.mox.ReplayAll()
        result = self.host_api.disable_zram(self.ctxt, 'fake_host', 'fake_action')
        self.assertEqual('fake-result', result)

    def test_get_zram_status(self):
        self._mock_rpc_call('get_zram_status',
                            host='fake_host',
                            action='fake_action')
        self.mox.ReplayAll()
        result = self.host_api.get_zram_status(self.ctxt, 'fake_host', 'fake_action')
        self.assertEqual('fake-result', result)

    def test_host_ksm_ctl(self):
        #self._mock_assert_host_exists()
        self._mock_rpc_call('host_ksm_ctl',
                            host='fake_host',
                            action='fake_action')
        self.mox.ReplayAll()
        
        result = self.host_api.host_ksm_ctl(self.ctxt, 'fake_host',
                                                 'fake_action')
        self.assertEqual('fake-result', result)
