...
class ServerActionsControllerTestV21(test.TestCase):
    ...
    def _get_actual_total_memory_mock(*args):
        memory_stats = {
            'actual': 2048*1024,
            'available': 1200*1024,
            'major_fault': 1024,
    ...
    def _test_actual_total_memory(self, case, size=-1):
        body = {
        ...
        if case == 'proper':
            self.set_actual_total_called = False

            def set_actual_total_memory_mock(*args):
                self.set_actual_total_called = True

            self.stubs.Set(compute_api.API,
                           'set_actual_total_memory',
                           set_actual_total_memory_mock)
        elif case == 'get':
            body = dict()

        self.stubs.Set(servers_v21.ServersController,
                       '_get_instance',
                       lambda *args: None)
        self.stubs.Set(servers_v21,
                       'authorize',
                       lambda *args: None)
        self.stubs.Set(compute_api.API,
                       'get_actual_total_memory',
                       self._get_actual_total_memory_mock)

        if case != 'get':
            def get_max_memory_mock(*args):
                return 2048 * 1024
        ...
        if case in ['big', 'small']:
            self.assertRaises(webob.exc.HTTPBadRequest,
                              self.controller._action_set_actual_total_memory,
                              self.req, FAKE_UUID, body=body)
        elif case == 'proper':
            self.controller._action_set_actual_total_memory(self.req, FAKE_UUID,
                                                            body=body)
            self.assertTrue(self.set_actual_total_called)

        elif case == 'get':
            result = self.controller._action_get_actual_total_memory(self.req, FAKE_UUID,
                                                                     body)
            self.assertJsonEqual(result, self._get_actual_total_memory_mock())

    def test_set_actual_total_memory_with_big_value(self):
        self._test_actual_total_memory('big', size=2049)

    def test_set_actual_total_memory_with_small_value(self):
        self._test_actual_total_memory('small', size=999)

    def test_set_actual_total_memory_with_proper_value(self):
        self._test_actual_total_memory('proper', size=1000)

    def test_get_actual_total_memory(self):
        self._test_actual_total_memory('get')
