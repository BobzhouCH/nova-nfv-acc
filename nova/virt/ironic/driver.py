...
class IronicDriver(virt_driver.ComputeDriver):
    ...
    def mac_of_mgmt(self, instance):
        ...
        ports = self.ironicclient.call("node.list_ports", node.uuid)
        for port in ports:
            port_detail = self.ironicclient.call('port.get', port.uuid)
    ...
    def spawn(self, context, instance, image_meta, injected_files,
        ...
        try:
            self._plug_vifs(node, instance, network_info, context=context)
    ...
    def _plug_vifs(self, node, instance, network_info, context=None):
        ...
        if len(network_info) - 1 > len(ports):
            ...
            for vif in network_info:
                for pif in ports:
                    if vif['address'] == pif.address:
                        ...
                        extra_value_name = '/extra/tenant_vif_port_id'
                        tenant_segment_id = None
                        if pif.address == inst_meta.get('bm_mgmt_mac',
                                None) and inst_meta.get('bm_mgmt_uuid',
                                None) == vif['network']['id']:
                            extra_value_name = '/extra/vif_port_id'
                        else:
                            # tenant network
                        ...
                        patch = [{'op': 'add',
                                  'path': extra_value_name,
                                  'value': port_id}]
                        if tenant_segment_id:
                            patch.append({'op': 'add',
                                  'path': '/extra/tenant_segment_id',
