...
def _numa_fit_instance_cell(host_cell, instance_cell, limit_cell=None, numa_mapping=None):
    ...
    if instance_cell.pagesize:
        ...
        if not pagesize:
            return
    pagesize = 2048
    # NFVI: Currently only support 2M pagesize, so force return 2M pagesize. Otherwise, this can lead to VM creation failure with 1G memoryb.
    # 2*1GB hugepage memory allocated to PMD thread of DPDK.  
...
def _numa_get_pagesize_constraints(flavor, image_meta):
    ...
    # NFVI: Because dpdk needs to use 2M large page,Force return 2M pagesize    
    return MEMPAGES_LARGE
...
def numa_get_flavor_node_map_list(flavor):
    # get vNUMA to pNUMA binding mapping
    # return [1,0] means 
    hw_numa_node = []
...
def numa_fit_instance_to_host(
    ...
    numa_mapping = []
    if flavor:
        numa_mapping = numa_get_flavor_node_map_list(flavor)
    if not (host_topology and instance_topology):
        ...
        if numa_mapping:
            ...
            for  instance_cell in instance_topology.cells:
                instance_cell.id = i
                i = i + 1
                host_cell = host_topology.cells[numa_mapping[instance_cell.id]]
                try:
                    got_cell = _numa_fit_instance_cell(
                        host_cell, instance_cell, limits, numa_mapping)
