...
from nova.virt import user_data
...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def block_info(self, instance, disk):
        guest = self._host.get_guest(instance)
        info = guest._domain.blockInfo(disk)
        return {'Capacity(KB)': info[0] / 1024
               ,'Allocation(KB)': info[1] / 1024
               ,'Physical(KB)': info[2] / 1024}

    def block_resize(self, instance, disk, size_in_kilobytes):
        guest = self._host.get_guest(instance)
        return guest._domain.blockResize(disk, size_in_kilobytes)

    def set_memory(self, instance, size, granularity):
        """Set actual total memory for the specified instance"""
        guest = self._host.get_guest(instance)
        size_in_kilobytes = size*1024*1024 if granularity == "GB" \
            else size*1024
        LOG.debug("size for setting memory: %s" % size_in_kilobytes)
        return guest._domain.setMemory(size_in_kilobytes)

    def get_memory(self, instance):
        """Get actual total memory for the specified instance"""
        guest = self._host.get_guest(instance)
        memory_stats = guest._domain.memoryStats()
        LOG.debug("The actual total memory for instance %s is %s KB"
                  % (str(instance.uuid), memory_stats['actual']))
        return memory_stats

    def get_max_memory(self, instance):
        """Get the max physical memory set to the specified instance"""
        guest = self._host.get_guest(instance)
        max_memory = guest._domain.maxMemory()
        LOG.debug("The max memory for instance %s is %s KB"
                  % (str(instance.uuid), max_memory))
        return max_memory
    ...
    def _create_image(self, context, instance,
        ...
        # File injection only if needed
            ...
            user_data.update_metadata(context, instance, admin_pass)
    ...
    def _get_guest_numa_config(self, instance_numa_topology, flavor, pci_devs,
        ...
        if not guest_cpu_numa_config:
            ...
            if topology:
                ...
                numa_mapping = hardware.numa_get_flavor_node_map_list(flavor)

                for host_cell in topology.cells:
                    for guest_node_id, guest_config_cell in enumerate(
                            guest_cpu_numa_config.cells):
                        if numa_mapping:
                            if numa_mapping[guest_node_id] == host_cell.id:
                                node = numa_memnodes[guest_node_id]
                                node.cellid = guest_node_id
                                node.nodeset = [numa_mapping[node.cellid]]

                                node.mode = "strict"

                                numa_mem.nodeset.append(node.nodeset[0])
                                ...
                                for cpu in guest_config_cell.cpus:
                                    ...
                                    if (object_numa_cell.cpu_pinning and
                                        ...
                                        if numa_mapping:
                                            pin_cpuset.cpuset = topology.cells[node.nodeset[0]].cpuset
    ...
    def _get_guest_config(self, instance, network_info, image_meta,
        ...
        # pvpanic device for lenovo test on qemu/kvm only
        auto_recovery_enabled = (flavor.extra_specs.get('hw:auto_recovery')
                           or 'disabled')
        LOG.debug("reaching pvpanic test %s", auto_recovery_enabled, instance=instance)
        if ( auto_recovery_enabled == "enabled" and (virt_type in ( 'qemu', 'kvm'))):
            pvpanic = vconfig.LibvirtConfigGuestPvpanic()
            guest.add_device(pvpanic)
    ...
    def pre_live_migration(self, context, instance, block_device_info,
        ...
        if not is_shared_instance_path:
            ...
            if os.path.exists(instance_dir):
                # NOTE(gcb) just remove existing directory, we can't decide
                # to remove it easily in other places.
    ...
    def _get_disk_over_committed_size_total(self):
        ...
        for dom in instance_domains:
            try:
                ...
                if guest.uuid not in local_instances:
                    LOG.error(_LE("The guest uuid %s is not in local_instances." %(guest.uuid)))
                if not (bdms and guest.uuid in bdms):
                    LOG.error(_LE("The guest uuid %s is not in bdms %s." %(guest.uuid,bdms)))
