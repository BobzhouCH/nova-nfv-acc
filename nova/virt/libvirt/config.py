...
class LibvirtConfigGuestPvpanic(LibvirtConfigGuestDevice):
    def __init__(self, **kwargs):
        super(LibvirtConfigGuestPvpanic, self).__init__(root_name="panic",
                                                         **kwargs)

        self.model = 'isa'

    def format_dom(self):
        dev = super(LibvirtConfigGuestPvpanic, self).format_dom()

        dev.set('model', self.model)
        address = self._new_node("address")
        address.set('type', 'isa')
        address.set('iobase', '0x505')
        dev.append(address)
...
class LibvirtConfigGuest(LibvirtConfigObject):
    ...
    def _format_powerActions(self, root):
        '''
        ...
        self.on_lockfailure = 'ignore'
        root.append(self._text_node("on_poweroff", self.on_poweroff))
        root.append(self._text_node("on_reboot", self.on_reboot))
        root.append(self._text_node("on_crash", self.on_crash))
        root.append(self._text_node("on_lockfailure", self.on_lockfailure))
    ...
    def format_dom(self):
        ...
        self._format_powerActions(root)
