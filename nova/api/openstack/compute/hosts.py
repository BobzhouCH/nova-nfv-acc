...
class HostController(wsgi.Controller):
    ...
    def _ksm_ctl(self, req, host_name, action):
        context = req.environ['nova.context']
        result = {'result': 'none'}
        LOG.error("ksm %s %s", host_name, action)
        result = self.api.host_ksm_ctl(context, host_name=host_name,
                action=action)
        return result

    @extensions.expected_errors((400, 404, 501))
    def enable_ksm(self, req, id):
        return self._ksm_ctl(req, id, "enable")

    @extensions.expected_errors((400, 404, 501))
    def disable_ksm(self, req, id):
        return self._ksm_ctl(req, id, "disable")

    @extensions.expected_errors((400, 404, 501))
    def show_ksm(self, req, id):
        return self._ksm_ctl(req, id, "show")
    ...
    @extensions.expected_errors((400, 404, 501))
    def enable_zram(self, req, id):
        context = req.environ['nova.context']
        result = {'result': 'none'}
        try:
            result = self.api.enable_zram(context, host_name=id, action="enable")
        except:
            common.raise_feature_not_supported()
        return result

    @extensions.expected_errors((400, 404, 501))
    def disable_zram(self, req, id):
        context = req.environ['nova.context']
        result = {'result': 'none'}
        try:
            result = self.api.disable_zram(context, host_name=id, action="disable")
        except:
            common.raise_feature_not_supported()
        return result

    @extensions.expected_errors((400, 404, 501))
    def get_zram_status(self, req, id):
        context = req.environ['nova.context']
        result = {'result': 'none'}
        try:
            result = self.api.get_zram_status(context, host_name=id, action="status")
        except:
            common.raise_feature_not_supported()
        return result
    ...
    def get_resources(self):
        resources = [extensions.ResourceExtension(ALIAS,
                ...
                member_actions={"startup": "GET", "shutdown": "GET",
                        "reboot": "GET", "enable_ksm": "GET",
                        "disable_ksm": "GET", "show_ksm": "GET",
                        "enable_zram": "GET", "disable_zram": "GET",
                        "get_zram_status": "GET"})]
        return resources
