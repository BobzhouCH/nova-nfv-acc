...
bm_opts = [
    cfg.StrOpt('bm_mgmt_uuid',
               default=None,
               help='Network used to manage baremetal machines'), ]

CONF.register_opts(bm_opts)
...
class ServersController(wsgi.Controller):
    ...
    def basic(self, req):
        ...
        try:
            servers = objects.InstanceList.get_all(context.elevated())
            servers = [{"id":s.uuid,
                        "name":s.display_name,
                        "status":common.status_from_state(s.vm_state,s.task_state),
                        "host":s.host,
                        #"flavor_id":s.flavor['id'], #will slow the api
                        "vcpus":s.vcpus,
                        "memory_mb":s.memory_mb,
                        "root_gb":s.root_gb,
                        "image_id":s.image_ref}
                        for s in servers ]
    ...
    @wsgi.response(200)
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('hot_extend')
    def _action_hot_extend(self, req, id, body):
        try:
            new_vdisk_size = body['hot_extend']
            size = new_vdisk_size['size']
            granularity = new_vdisk_size.get('granularity', "GB").upper()
            if granularity != "GB":
                raise webob.exc.HTTPBadRequest(explanation='Granularity should be GB or None!')

            context = req.environ['nova.context']
            instance = self.compute_api.get(context, id, want_objects=True, expected_attrs=['flavor'])
            old_gb = instance.root_gb
            if int(old_gb) > int(size):
                raise webob.exc.HTTPBadRequest(explanation='New disk size should be larger than the old size!')
            authorize(context, instance, 'hot_extend')
            disk = None
            r = self.compute_api.hot_extend_vdisk(context, instance, disk, size, granularity)
            if r is not None:
                raise webob.exc.HTTPBadRequest(explanation=r)
    ...
    @wsgi.response(200)
    @extensions.expected_errors((400, 404, 409))
    @wsgi.action('set_actual_total_memory')
    def _action_set_actual_total_memory(self, req, id, body):
        """Set actual total memory for the specified instance."""
        try:
            set_memory_dict = body['set_actual_total_memory']
            size = int(set_memory_dict['size'])
            granularity = set_memory_dict.get('granularity', "MB").upper()
            granularity = granularity if granularity in ["MB", "GB"] else "MB"

            context = req.environ['nova.context']
            instance = self._get_instance(context, id)
            authorize(context, instance, 'set_actual_total_memory')

            # validation for the input size
            mem_stats = self.compute_api.get_actual_total_memory(context, instance)
            used = mem_stats['rss']
            max_memory = self.compute_api.get_max_memory(context, instance)
            MIN = int(min(max(used*1.15, used+200*1024), max_memory)/1024.0) + 1
            MAX = int(max_memory/1024.0)
            if MIN <= size <= MAX:
                self.compute_api.set_actual_total_memory(context, instance,
                                                         size, granularity)
            else:
                err = "The recommended memory scope is from %s MB to %s MB" \
                      % (MIN, MAX)
                raise exception.InvalidInputForSettingActualTotalMemory(reason=err)
            ...
            common.raise_http_conflict_for_instance_invalid_state(state_error,
                                                                  'set_actual_total_memory',
                                                                  id)
        except exception.InvalidInputForSettingActualTotalMemory as e:
            raise webob.exc.HTTPBadRequest(explanation=e.format_message())

    @wsgi.response(200)
    @extensions.expected_errors((404, 409))
    @wsgi.action('get_actual_total_memory')
    def _action_get_actual_total_memory(self, req, id, body):
        """Get actual total memory for the specified instance."""
        try:
            context = req.environ['nova.context']
            instance = self._get_instance(context, id)
            authorize(context, instance, 'get_actual_total_memory')
            mem_stats = self.compute_api.get_actual_total_memory(context, instance)
            mem_stats['granularity'] = 'KB'
            return mem_stats
...
class Servers(extensions.V21APIExtensionBase):
    ...
    def get_resources(self):
        ...
        collection_actions = {'detail': 'GET','basic':'GET'}
        resources = [
