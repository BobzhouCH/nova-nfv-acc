...
class API(base_api.NetworkAPI):
    ...
    def _create_port(self, port_client, instance, network_id, port_req_body,
        ...
        try:
            ...
            # set a default port name for horizon display port details
            port_req_body['port']['name'] = "nic_" + str(time.time())
    ...
    def deallocate_for_instance(self, context, instance, **kwargs):
        ...
        # delete the nic that created by nova instance begin
        nova_nic = []
        is_vnic = re.compile(r'^nic_')
        for port in data.get('ports', []):
            if is_vnic.match(port.get('name')):
                nova_nic += [port['id']]
        # delete the nic that created by nova instance end
    ...
    def _format_floating_ip_model(self, fip, pool_dict, port_dict):
        ...
        # NOTE(gcb) We add specific code for easystack
        # fip['port_id'] may not in port_dict
        if fip['port_id'] and fip['port_id'] in port_dict:
    ...
    def _nw_info_build_network(self, port, networks, subnets):
        network_name = None
        network_mtu = None
        network_segmentation_id = None
        for net in networks:
            if net.get('provider:network_type') == 'vlan':
                network_segmentation_id = net.get('provider:segmentation_id')
        ...
        if network_segmentation_id:
            network['vmware_neutron_bridge'] = 'vlan-' + str(network_segmentation_id)
            network['vmware_neutron_vlan'] = network_segmentation_id
